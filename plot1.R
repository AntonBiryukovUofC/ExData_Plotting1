# This script reproduces plot1.R
...
tmp <- read.table("./txt_file/household_power_consumption.txt",header = TRUE, 
                  nrows = 1,na.strings = "?", sep = ";" )
epc <- tbl_df(read.table("./txt_file/household_power_consumption.txt",header = TRUE, 
                  #nrows = 1000,
                  na.strings = "?", sep = ";", 
...
data_to_plot <- tbl_df(read.table("./txt_file/household_power_consumption.txt",header = TRUE,na.strings = "?",
                                  sep = ";" )[epc$id_to_import,])
data_to_plot <- data_to_plot %>% mutate(Date = dmy(Date)) %>% mutate (Time = hms(Time))
}
# Do plotting
par(mfrow = c(1,1))
with(data_to_plot, {hist(Global_active_power,col = "red", main = "Global Active Power",
